...
void Hallway::ResetEnv()
{
	...
	double obs_x = -2 + rand()%16;     //default value
	...
	for(int i=0; i<agent_num; i++)
	{
		...
		{
			if(i%2==0){
				agent_pos[0] = obs_x + 6.0 + (rand()%20)*0.1;
				// agent_pos[0] = 0.0;
				...
				agent_pos[0] = -20.0 + (rand()%400)*0.1;   //I changed it to create the red agents at the more left side
				agent_pos[1] = -14 + (rand()%20)*0.1;
		...
		while(true){
			if(i%2==0){
				...
				d_pos[0] = -20 + rand()%40;  // //I changed it to create the destinations of the right side to the little more left side so that the don't get lost from the window
				d_pos[1] = 10 + rand()%2;
