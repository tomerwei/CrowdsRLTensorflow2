...
void Hallway::ResetEnv()
{
	...
	double obs_x = -4 + rand()%16;     //default value
	...
	for(int i=0; i<agent_num; i++)
	{
		...
		while(true)
		{
			if(i%2==0){
				...
				agent_pos[0] = -20.0 + (rand()%160)*0.1;   //I changed it to create the red agents at the more left side
				agent_pos[1] = -2.5 + (rand()%50)*0.1;
		...
		while(true){
			if(i%2==0){
				...
				d_pos[0] = -23 + rand()%5;  //I changed it to create the destinations of the left side to the little more right side so that the don't get lost from the window
				d_pos[1] = -5 + rand()%10;
				...
				d_pos[0] = 18 + rand()%4;  // //I changed it to create the destinations of the right side to the little more left side so that the don't get lost from the window
				d_pos[1] = agent_pos[1];
